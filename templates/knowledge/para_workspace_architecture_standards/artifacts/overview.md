...
To prevent confusion between platform-generated metadata and workspace operational artifacts:
- **Implementation Plan:** Defined strictly as the plan file generated by the workspace `/plan` workflow (located at `Projects/<slug>/artifacts/plans/plan-*.md`).
- **Tasks:** Checklists and items defined directly within the active implementation plan or backlog in the workspace.
- **Execution Rule:** AI Agents must code and execute based solely on workspace-defined plans and tasks.
- **Brain Artifact Isolation:** AI Agents **MUST NOT** align execution with ephemeral IDE-internal files (such as those in `~/.gemini/antigravity-ide/brain/<conversation-id>/task.md` or `walkthrough.md`). These brain artifacts are platform-specific telemetry, not the authority for the workspace code execution.
