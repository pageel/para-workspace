...
- [ ] 2. 🔴 **Verify FAIL**
  ...
  ```
  - Expected: FAIL due to missing code/logic. Evidence auto-logged to `.beads/tdd-evidence.log`.
...
- [ ] 5. 🤖 **TDD Gate** — Agent reads `.beads/tdd-evidence.log`, verifies FAIL entry exists before PASS entry for this test file. If not compliant → STOP, do not commit.
- [ ] 6. 👤 **Commit**
  ```bash
...
- [ ] 5. 🤖 **TDD Gate** — verify evidence log: FAIL before PASS
...
- [ ] 2. 🟢 **Verify PASS:** Re-run entire test suite to ensure no regressions.
- [ ] 3. 👤 **Commit Checkpoint**
  ```bash
...
> **Audit trail:** TDD compliance is verified via `.tdd-evidence.log` (auto-generated by
