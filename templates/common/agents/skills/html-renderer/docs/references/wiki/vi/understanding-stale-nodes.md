...
### Bước 2: Biên dịch lại Đồ thị mã nguồn (Rebuild Graph)
Chạy lệnh biên dịch đồ thị để tự động quét lại toàn bộ neo liên kết và cập nhật trạng thái mới nhất:
```bash
...
```
Lệnh này quét toàn bộ neo `<!-- @graph-node -->` trong tài liệu Markdown, đối chiếu với comment code và tái thiết lập các liên kết neo của đồ thị.
...
*   **Tự động cập nhật tài liệu đồng bộ với thay đổi mã nguồn**:
    ...
    ```
*   **Biên dịch lại đồ thị mã nguồn để cập nhật neo**:
    ```text
    /para para-graph build [project-name]
    ```
