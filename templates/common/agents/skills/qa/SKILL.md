...
- **[Spec]** Does the spec define a standardized error response format (e.g., `{ error: string, code: string }`) for all API boundaries? What happens when an API returns 500 — JSON or raw HTML?
- **[Spec]** Are concurrent access scenarios addressed (e.g., two users modifying the same resource — last-write-wins, optimistic locking, or conflict detection)?
...
1.  **Ingest Spec & Sysdesign:** Read the entire target specification and the corresponding system design file (if available).
2.  **Evaluate 30 Checklist Items:** Run the 30 checks from `spec-quality-checklist.md`.
3.  **Cross-Check Database, APIs, and Spec Overlap:**
    ...
    *   Scan existing specs in `artifacts/specs/` to verify boundaries between the target Spec and other specs are clear and non-conflicting.
4.  **Validate CSA Anchors:** Scan the anchors and ensure no blanket anchors are used at the H1/H2 levels.
5.  **Output Report:** Create the QA report and propose spec fixes directly to the user.
