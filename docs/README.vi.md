...
| [📑 Workflow Catalog](#-catalog-workflow) | 23 governed workflows |
| [🛡️ Rule Catalog](#️-catalog-quy-tắc-rules) | 11 governance rules |
| [🧩 Skill Catalog](#-catalog-kỹ-năng-skills) | 2 reusable skills |
...
| H7  | Tham chiếu xuyên project + Ecosystem (v1.6.0)           |
...
| **[`/knowledge`](./workflows/knowledge.md)**         | Quản lý Knowledge Items — dashboard, tạo, kiểm tra, lưu trữ |
...
```
  ...
  Step 2.5b: LUÔN đọc workspace skills index  (v1.6.2+)
  ┌───────────────────────────────────────────────────┐
  │  .agents/skills.md  (~10 dòng, ~100 tokens)        │
  │  Agent ghi nhớ triggers → tải skill KHI CẦN       │
  └───────────────────────────────────────────────────┘

  Step 2.5c: CÓ ĐIỀU KIỆN đọc project agent indices
  ┌───────────────────────────────────────────────────┐
  │  Kiểm tra project.md:                              │
  ...
  💡 Tổng chi phí: ~350 tokens (so với ~2000 nếu tải hết)
```
...
```
Lớp    Cơ chế                        Vị trí                          Sống sót?
────    ────────────────────────      ────────────────────────────    ─────────
 1      Hướng dẫn trong index          agent-behavior.md §4            ⚠️ Mất sau truncation
 2      Safety block trong output     /open Step 8 báo cáo            ✅ Trong checkpoint summary
 3      Workflow pre-flight           Step 0: đọc lại rules + skills  ✅ Đọc từ đĩa
 4      File guard headers            <!-- ⚠️ ... --> trong file     ✅ Inline trong file đích
```
...
```
Hội thoại dài → Context window bị cắt → Agent mất rules
                                                 │
                            ┌────────────────────┤
                            ▼                    ▼
                   Agent chạy /push        Agent sửa done.md
                       │                        │
...
┌──────────┐                ┌──────────┐                ┌──────────┐
│ Agent    │                │ Agent    │                │ Agent    │
│ phát hiện│──── lưu ────▶ │ tự động  │──── tinh ───▶ │ áp dụng  │
│ insight  │    làm KI      │ được load│    chỉnh       │ ngay lập │
└──────────┘                └──────────┘    tức         └──────────┘
```
...
| `/knowledge` | Vòng đời đầy đủ: tạo, cập nhật, kiểm tra, lưu trữ |
