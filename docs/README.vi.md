...
PARA Workspace giải quyết bài toán "Lãng phí Token & Mất trí nhớ" của AI Agent thông qua kiến trúc **Hybrid 3-File Model** (Giới thiệu ở v1.5.1, Working Checkmarks ở v1.5.2).
...
3. **Working Checkmarks** (v1.5.2): Agent ĐƯỢC PHÉP tick `[x]` trong `sprint-current.md` khi code — giống hệt Planning Mode. Khi đóng phiên (`/end`) hoặc chạy `/backlog update`, checkmarks tự động reconcile về `backlog.md`.
   > _Lợi ích: Agent code liên tục không bị gián đoạn, tiến độ luôn được ghi nhận dù quên chạy `/backlog update`._
