...
Lệnh này sẽ `git pull` repo, chạy migration phân tầng theo phiên bản (chỉ những bước cần thiết), và đồng bộ lại tất cả governed libraries. File tuỳ chỉnh được sao lưu thành `.bak` trước khi ghi đè (Smart Sync).
...
3. `install.sh` đồng bộ kernel, workflows, rules, skills
...
| Vấn đề                            | Giải pháp                                                                                                  |
| :-------------------------------- | :--------------------------------------------------------------------------------------------------------- |
| **macOS: permission denied**      | Chạy `chmod +x` cho CLI scripts (Bước 2 bên trên)                                                          |
