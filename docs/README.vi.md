...
| **[`agent-behavior`](./rules/agent-behavior.md)**                                 | Giao tiếp, ngôn ngữ, Context Recovery (v1.5.4)          | 🟡 Important |
| **[`context-rules`](./rules/context-rules.md)**                                   | Loading order, isolation, Two-Tier trigger                | 🟡 Important |
...
### Defense-in-Depth: 4 Lớp Bảo vệ Rule

Sau cuộc hội thoại dài, AI agent mất context (truncation). PARA Workspace có **4 lớp độc lập** đảm bảo rules luôn sống sót:

```
Lớp    Cơ chế                        Vị trí                          Sống sót?
────    ────────────────────────      ────────────────────────────    ─────────
 1      Hướng dẫn trong rule file    agent-behavior.md §4            ⚠️ Mất sau truncation
 2      Safety block trong output    /open Step 8 báo cáo            ✅ Trong checkpoint summary
 3      Workflow pre-flight          Step 0: đọc lại rules.md       ✅ Đọc từ đĩa
 4      File guard headers           <!-- ⚠️ APPEND-ONLY (C2) -->   ✅ Inline trong file đích
```
...
> 📖 Chi tiết kiến trúc: [Context Recovery](./architecture/context-recovery.md) · [Rule Layers](./architecture/rule-layers.md)
