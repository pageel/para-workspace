...
内置 26 个受治理核心工作流（如 `/plan`, `/backlog`, `/open`, `/end`, `/para-knowledge`, `/para-skill`, `/write`, `/logs` 等）用于规范化 AI Agent 在项目内的操作流。
...
目前包含 8 个复用技能（`PARA Kit`, `Formatting`, `Page Map`, `PARA Skill`, `Plan Templates`, `Docs Templates`, `Brainstorm Templates`, `Write Templates`），按需加载并提供模板、模式和参考资料。
...
当前版本为 **1.7.14** (Content Authoring Ecosystem & Session Telemetry)。
